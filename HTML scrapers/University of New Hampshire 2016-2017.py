...
URL = "http://www.unh.edu/archive/undergrad-catalog/2016-2017/courselist.cfm.html"
...
for link in results:
    ...
    p =  re.search(r'ug-[a-z]*-[0-9]*.htm', str(link))
    #could possible do something with the first group?
    if p:
        departments.append("http://www.unh.edu/archive/undergrad-catalog/2016-2017/1617/" + p.group(0))
...
for dep in departments:    
    ...
    page = requests.get(URL)
    soup = BeautifulSoup(page.text, 'html.parser')
    depname = soup.find(class_="ddheader")
    if depname is None: 
        continue
    depname = depname.text

    number = soup.findAll("p")
    print(number)
    break
    #desc = soup.findAll("br")
